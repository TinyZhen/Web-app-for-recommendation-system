...
export default function PersonalInfoForm({ user, onNext, error, setError }) {
  const [gender, setGender] = useState("");
  const [age, setAge] = useState(null);          // store as integer
  const [occupation, setOccupation] = useState(null); // store as integer
  const [zipcode, setZipcode] = useState("");
  ...
  async function handleSubmit(e) {
    ...
    try {
      await setDoc(
        ...
        {
          gender,
          age: age ?? null,               // already integer
          occupation: occupation ?? null, // already integer
          zipcode,
