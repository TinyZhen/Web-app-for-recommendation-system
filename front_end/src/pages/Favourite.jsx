...
export default function Favourites() {
  ...
  async function handleDelete(id) {
    try {
      await deleteDoc(doc(db, "savedRecommendations", id)); // delete from Firestore
      setItems((prev) => prev.filter((item) => item.id !== id)); // update UI
    } catch (e) {
            ...
            items.length === 0 ? (
              ...
              >
                {items.map((rec) => (
                  <SavedRecommendCard
                    key={rec.id}
                    id={rec.id}                     // <-- ✅ CHANGE: passing id
                    text={rec.explanation}
                    onDelete={handleDelete}         // <-- ✅ CHANGE: passing delete handler
                  />
                ))}
              </div>
            )
